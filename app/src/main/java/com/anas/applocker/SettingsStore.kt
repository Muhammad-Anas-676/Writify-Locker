...
class SettingsStore(context: Context) {
    ...
    // This is intentionally NOT a "shown once" flag. It only becomes true when the user
    // explicitly says they've done the manual OEM steps - dismissing the dialog or just
    // opening the settings screen does NOT count, since there's no public API to verify
    // it actually got enabled. If the keep-alive heartbeat later finds the service dead
    // again, this gets reset to false automatically so the guide comes back.
