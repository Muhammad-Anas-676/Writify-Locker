...
class SettingsActivity : AppCompatActivity() {
    ...
    private val pickCustomIconLauncher =
        registerForActivityResult(ActivityResultContracts.GetContent()) { uri ->
            if (uri != null) requestPinCustomShortcut(uri)
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupFakeStorageRow()
        setupCustomShortcutRow()

        findViewById<TextView>(R.id.changePinRow).setOnClickListener { showChangePinDialog() }
        findViewById<TextView>(R.id.autostartGuideRow).setOnClickListener { AutostartGuideHelper.show(this) }
    ...
    // ---------- Behavior toggles (items 2, 5, 4, 7, 8, 10, 9 from the spec) ----------

    private fun setupBehaviorSwitches() {
        bindSwitch(R.id.instantRelockSwitch, settingsStore.isInstantRelockOnSwitch()) {
            settingsStore.setInstantRelockOnSwitch(it)
        }
        bindSwitch(R.id.rotaryHapticsSwitch, settingsStore.isRotaryHapticsSoundEnabled()) {
            settingsStore.setRotaryHapticsSoundEnabled(it)
        }
        bindSwitch(R.id.stealthRecentsSwitch, settingsStore.isStealthRecentsEnabled()) {
            settingsStore.setStealthRecentsEnabled(it)
        }
        bindSwitch(R.id.shakeToLockSwitch, settingsStore.isShakeToLockEnabled()) {
            settingsStore.setShakeToLockEnabled(it)
        }
        bindSwitch(R.id.flipToExitSwitch, settingsStore.isFlipToExitEnabled()) {
            settingsStore.setFlipToExitEnabled(it)
        }
        bindSwitch(R.id.biometricFallbackSwitch, settingsStore.isBiometricFallbackEnabled()) {
            settingsStore.setBiometricFallbackEnabled(it)
        }
        bindSwitch(R.id.autoClearLogsSwitch, settingsStore.isAutoClearBreakInLogsEnabled()) {
            settingsStore.setAutoClearBreakInLogsEnabled(it)
            if (it) pinManager.clearBreakInLog()
        }
    }
    ...
    // ---------- Fake storage notification (item 3 from the spec) ----------

    private fun setupFakeStorageRow() {
        bindSwitch(R.id.fakeStorageNotifSwitch, settingsStore.isFakeStorageNotificationEnabled()) {
            settingsStore.setFakeStorageNotificationEnabled(it)
        }
        findViewById<TextView>(R.id.sendTestNotificationRow).setOnClickListener {
            FakeStorageNotifier.show(this)
            Toast.makeText(this, "Notification sent", Toast.LENGTH_SHORT).show()
        }
    }

    // ---------- Custom app icon / name shortcut (spec section 4) ----------

    private fun setupCustomShortcutRow() {
        findViewById<TextView>(R.id.pickCustomIconRow).setOnClickListener {
            val name = findViewById<EditText>(R.id.customShortcutNameInput).text.toString()
            if (name.isBlank()) {
                Toast.makeText(this, "Enter a shortcut name first", Toast.LENGTH_SHORT).show()
    ...
    private var pendingShortcutName: String = ""

    private fun requestPinCustomShortcut(uri: Uri) {
        val ok = CustomShortcutHelper.requestCustomShortcut(this, uri, pendingShortcutName)
        Toast.makeText(
            this,
            if (ok) "Shortcut requested - confirm on your home screen" else "Couldn't create shortcut",
            Toast.LENGTH_LONG
        ).show()
    }
    ...
    private fun readBackup(uri: android.net.Uri) {
        try {
            ...
            if (lockedAppsJson != null) {
                val packages = (0 until lockedAppsJson.length()).map { lockedAppsJson.getString(it) }
                lockedAppsStore.setLockedBulk(packages, true)
            }
